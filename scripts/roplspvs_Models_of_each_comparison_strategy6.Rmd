...
directory_and_filename_rdata <-paste(directory_output_reports, paste(paste(projectname,date_of_analysis, group1, "vs", group2, secID,sep="_"),".Rdata", sep = ""),sep="/")
filename_rdata <- paste(paste(projectname, date_of_analysis, group1, "vs", group2, secID,sep="_"),".Rdata", sep = "")
...
plot(ropls_model_6_ortho0, typeVc = "loading") # Loadings plot
```
...
sig_graph_ortho0 <- ggplot(ropls_data_sig_ortho0, aes(x = reorder(Feature, correlation), y = correlation)) +
  ...
  labs(title = "Features in ortho 0",
       x = "",
       y = "p(corr)") +
    geom_errorbar(aes(ymin = correlation - se, ymax = correlation + se), width = 0.2)
...
plot(ropls_model_6_ortho1, typeVc = "loading") # Loadings plot
```
...
sig_graph_ortho1 <- ggplot(ropls_data_sig, aes(x = reorder(Feature, correlation), y = correlation)) +
  geom_bar(stat = "identity", fill="maroon") +
  coord_flip() + 
   theme(axis.text.y = element_text(size = 3)) +
...
plot(ropls_model_6_ortho2, typeVc = "loading") # Loadings plot
```
...
sig_graph_ortho1 <- ggplot(ropls_data_sig_ortho2, aes(x = reorder(Feature, correlation), y = correlation)) +
  geom_bar(stat = "identity", fill="maroon") +
  coord_flip() + 
   theme(axis.text.y = element_text(size = 3)) +
